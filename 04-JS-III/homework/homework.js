...
function numeroMasGrande(numeros) {
  ...
  for (let i=0; i<numeros.length; i++) {
    if (numeros[i]>bigest_number){
      bigest_number = numeros[i];
    }
  }
...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  for (let i=0; i<arguments.length; i++) {
    multiplication_result = multiplication_result * arguments[i];
  }
...
function diaDeLaSemana(numeroDeDia) {
  ...
  if ((week_days[numeroDeDia-1]==="Sabado") || (week_days[numeroDeDia-1]==="Domingo")) {
...
function todosIguales(arreglo) {
  ...
  let first_element = arreglo[0]
  for (let i=1; i<arreglo.length; i++) {
    if (first_element !== arreglo[i]) {
      return false
    }
  }
